...
class IceBackendModule
{
  ...
  /**
   * Return the user actions from the ice_backend configuration
   *
   * @return array
   */
  ...
  /**
   * Return the items from the ice_backend configuration
   *
   * @return array
   *
   * @see iceBackendModule::initItem()  All items are initialized before being returned through this method
   * @see iceBackendModule::getProperty()
   */
  public static function getItems()
  {
    $items = self::getProperty('items', array());
    array_walk($items, 'iceBackendModule::initItem');
   ...
   * @return array
   *
   * @see iceBackendModule::getItems()
   * @see iceBackendModule::getCategories()
   * @see iceBackendModule::getProperty()
   */
  ...
  public static function getCategories()
  {
    ...
    foreach ($categories as $category_name => $category_data)
    {
      ...
      {
        array_walk($categories[$category_name]['items'], 'iceBackendModule::initItem');
      }
    }
  ...
  {
    return sfConfig::get('app_ice_backend_'. $name, $default);
  }
  ...
  {
    sfConfig::set('app_ice_backend_'.$name, $value);
  }
  ...
  /**
   * Get the current module name (as defined in the iceBackendModule configuration), if possible, with translation
  ...
  {
    $image = isset($item['image']) ? $item['image'] : iceBackendModule::getProperty('default_image');
    $image = (substr($image, 0, 1) == '/') ? $image : (iceBackendModule::getProperty('image_dir') . $image);
